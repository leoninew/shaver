<template>
  <q-layout view="lHh Lpr lFf">
    <q-header elevated>
      <q-toolbar>
        <q-btn flat dense round icon="menu" aria-label="Menu" />
        <q-toolbar-title>
          <q-breadcrumbs-el label="Leon's Shaver" to="/" />
        </q-toolbar-title>
        <div>
          <q-breadcrumbs-el label="Github" href="https://github.com/leoninew/shaver/tree/develop" target="_blank" />
        </div>
      </q-toolbar>
    </q-header>

    <q-page-container>
      <div class="q-pa-md">
        <div class="row">
          <div class="col-2">
            <q-list dense separator>
              <q-expansion-item v-for="item in transforms"
                                dense default-opened hide-expand-icon
                                active-class="list-item-active"
                                :to="item.to" :label="item.title">
                <q-list dense separator v-if="item.children">
                  <q-expansion-item v-for="item2 in item.children"
                                    dense default-opened hide-expand-icon
                                    active-class="list-item-active"
                                    :header-inset-level="0.3"
                                    :to="item2.to" :label="item2.title">
                  </q-expansion-item>
                </q-list>
              </q-expansion-item>
            </q-list>
          </div>

          <div class="col">
            <template v-if="isContainerPage" v-for="group in onlineToolsGroups">
              <div class="text-h5 text-uppercase">{{ group.key }}</div>
              <div class="q-pa-md row items-start q-gutter-md">
                <q-card v-for="item in group.items" style="max-width: 350px">
                  <q-card-section>
                    <div class="text-h6 text-primary">
                      <q-breadcrumbs-el :label="item.title" target="_blank" :href="item.href" />
                    </div>
                    <img v-if="item.cover" alt="" :src="item.cover" style="max-width: 100px;">
                    <div class="text-subtitle2">{{ item.description }}</div>
                  </q-card-section>
                </q-card>
              </div>
            </template>

            <template v-else>
              <router-view />
            </template>
          </div>
        </div>
      </div>
    </q-page-container>
    <q-footer elevated></q-footer>
  </q-layout>
</template>

<script setup lang="ts">
  import { onMounted, ref } from 'vue'
  import { onBeforeRouteUpdate, useRoute } from 'vue-router'
  import enumerable from 'enumerable-es'

  const route = useRoute()
  const isContainerPage = ref(true)

  type Transform = { to: string, title: string };
  type TransformGroup = Transform & { children?: Transform[] };

  const transforms: Array<TransformGroup> = [
    {
      to: '#',
      title: 'to C#',
      children: [
        {
          to: '/csharp?kind=type',
          title: 'Poco class',
        },
        {
          to: '/csharp?kind=rest',
          title: 'ASP.NET Core Controller',
        }
      ]
    },
    {
      to: '#',
      title: 'to JAVA',
      children: [
        {
          to: '/java?kind=type',
          title: 'Pojo class',
        },
        {
          to: '/java?kind=rest',
          title: 'SpringBoot Controller',
        },
        {
          to: '/java?kind=mybatisplus',
          title: 'MyBatis-Plus mapper and service',
        }
      ]
    },
    {
      to: '#',
      title: 'to TypeScript',
      children: [
        {
          to: '/typescript?kind=type',
          title: 'Type definition',
        },
        {
          to: '/typescript?kind=rest',
          title: 'REST API based on axios',
        }
      ]
    },
    {
      to: '/python',
      title: 'to Python'
    },
    {
      to: '/mysql',
      title: 'to MySQL'
    },
    {
      to: '/xml',
      title: 'to XML'
    },
    {
      to: '/yaml',
      title: 'to YAML'
    },
  ]

  type OnlineTool = { group: string, title: string, href: string, description: string, cover?: string };

  const onlineTools: Array<OnlineTool> = [
    {
      group: 'language',
      title: 'JSON Editor Online',
      href: 'https://jsoneditoronline.org/',
      description: 'JSON Editor Online is a web-based tool to view, edit, format, repair, compare, query, transform, validate, and share your JSON data.',
    },
    {
      group: 'language',
      title: 'JSONPath Online Evaluator',
      href: 'https://jsonpath.com/',
      description: 'A playground for JSONPath',
    },
    {
      group: 'diagrams',
      title: 'Mermaid Live Editor',
      href: 'https://mermaid.live/edit',
      description: 'Generation of diagrams like flowcharts or sequence diagrams from text in a similar manner as markdown',
    },
    {
      group: 'language',
      title: 'Transform',
      href: 'https://transform.tools/',
      description: 'A polyglot web converter.',
    },
    {
      group: 'language',
      title: 'Regex101',
      href: 'https://regex101.com/',
      description: 'Regular expression tester with syntax highlighting, explanation, cheat sheet for PHP/PCRE, Python, GO, JavaScript, Java, C#/.NET, Rust.',
      cover: 'https://camo.githubusercontent.com/668d52234b738c0b196be9bfd071a7845f0d27230dd11c040f8e4a0c268d51c0/68747470733a2f2f72656765783130312e636f6d2f7374617469632f6173736574732f69636f6e2d3135322e706e67'
    },
    {
      group: 'language',
      title: 'SQLFormat',
      href: 'https://sqlformat.org/',
      description: 'Online SQL formatting service',
    },
    {
      group: 'docker',
      title: 'Composerize',
      href: 'https://www.composerize.com/',
      description: '🏃→🎼 docker run asdlksjfksdf > docker-composerize up',
    },
    {
      group: 'language',
      title: 'ExplainShell',
      href: 'https://explainshell.com/',
      description: 'write down a command-line to see the help text that matches each argument',
    },
    {
      group: 'language',
      title: 'Online Yaml Tools',
      href: 'https://onlineyamltools.com/',
      description: 'World\'s simplest collection of useful YAML utilities. Convert YAML to XML, JSON, TSV and CSV. Validate, prettify, minify, highlight, edit YAML, and more.',
    },
    {
      group: 'diagrams',
      title: 'dbdiagram.io',
      href: 'https://dbdiagram.io/home/',
      description: 'Draw Entity-Relationship Diagrams, Painlessly 😎',
    },
    {
      group: 'diagrams',
      title: 'draw.io',
      href: 'https://app.diagrams.net/',
      description: 'draw.io is free online diagram software for making flowcharts, process diagrams, org charts, UML, ER and network diagrams.',
      cover: 'data:image/png;base64,'
    },
    {
      group: 'diagrams',
      title: 'PlantUML Editor',
      href: 'https://plantuml-editor.kkeisuke.dev/',
      description: 'PlantUML Editor. New UML. behavioral diagrams; Use Case; Activity β; Activity; Sequence. structural diagrams; Object; Class; ER diagram.',
    },
    {
      group: 'docker',
      title: 'kube-score',
      href: 'https://kube-score.com/',
      description: 'Kubernetes object analysis with recommendations for improved reliability and security.',
      cover: 'https://kube-score.com/logo.svg'
    },
    // {
    //   group: "",
    //   title: "",
    //   href: "",
    //   description: "",
    // },
  ]

  const onlineToolsGroups = ref<any>([])

  onMounted(() => {
    isContainerPage.value = route.path === '/'
    onlineToolsGroups.value = enumerable.from(onlineTools)
      .groupBy(x => x.group, x => x)
      .select(x => ({
        key: x.key(),
        items: x.toArray()
      }))
      .toArray()
  })

  onBeforeRouteUpdate((to) => {
    isContainerPage.value = to.path === '/'
  })
</script>

<style scoped>
.list-item-active {
  background: linear-gradient(to right, lightgray, lightgray 50%);
}
</style>
